...
| `router.ex` | HTTP: viewer + assets, `GET /content` (JSON snapshot, sets focus = newest), `GET /events` (SSE), `GET /media/<name>` (author images), `GET /download/<name>` (raw Markdown), `POST /push?tab=`, `POST /clear-old` (prune stale docs). |
